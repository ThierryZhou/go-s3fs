...
import (
	...
	"github.com/ThierryZhou/go-s3fs/fs/march"
...
func newSyncCopyMove(ctx context.Context, fdst, fsrc fs.Fs, deleteMode fs.DeleteMode, DoMove bool, deleteEmptySrcDirs bool, copyEmptySrcDirs bool) (*syncCopyMove, error) {
	...
	var err error
	s.toBeChecked, err = newPipe(ci.OrderBy, backlog)
	if err != nil {
		return nil, err
	}
	s.toBeUploaded, err = newPipe(ci.OrderBy, backlog)
	if err != nil {
		return nil, err
	}
	s.toBeRenamed, err = newPipe(ci.OrderBy, backlog)
	if err != nil {
		return nil, err
	}
	if ci.MaxDuration > 0 {
...
func (s *syncCopyMove) run() error {
	...
	if s.deleteMode != fs.DeleteModeOnly && s.currentError() == nil {
