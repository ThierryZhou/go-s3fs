...
import (
	...
	"github.com/ThierryZhou/go-s3fs/fs/config/configflags"
	"github.com/ThierryZhou/go-s3fs/fs/filter/filterflags"
	"github.com/ThierryZhou/go-s3fs/fs/log/logflags"
...
// Root is the main s3fs command
var Root = &cobra.Command{
	Use:   "s3fs",
	Short: "Show help for cobra commands, flags and backends.",
	Long: `
s3fs mounting remote files in local filesystem.
`,
	PersistentPostRun: func(cmd *cobra.Command, args []string) {
		fs.Debugf("s3fs", "Version %q finishing with parameters %q", fs.Version, os.Args)
	},
	DisableAutoGenTag: true,
}

// GeneratingDocs is set by s3fs gendocs to alter the format of the
...
var helpFlags = &cobra.Command{
	...
	Short: "Show the global flags for s3fs",
	Run: func(command *cobra.Command, args []string) {
		if len(args) > 0 {
...
// runRoot implements the main s3fs command with no subcommands
...
func setupRootCommand(rootCmd *cobra.Command) {
	...
	cobra.AddTemplateFunc("showLocalFlags", func(cmd *cobra.Command) bool {
		// Don't show local flags (which are the global ones on the root) on "s3fs" and
		// "s3fs help" (which shows the global help)
		return cmd.CalledAs() != "s3fs" && cmd.CalledAs() != ""
	})
...
Use "s3fs help flags" for to see the global flags.
Use "s3fs help backends" for a list of supported services.
`
...
This describes the global flags available to every s3fs command
...
func showBackend(name string) {
	...
	for _, opts := range []fs.Options{standardOptions, advancedOptions} {
		...
		for _, opt := range opts {
			...
			fmt.Printf("%s\n\n", opt.Help)
			if opt.IsPassword {
				fmt.Printf("**NB** Input to this must be obscured - see [s3fs obscure](/commands/s3fs_obscure/).\n\n")
			}
