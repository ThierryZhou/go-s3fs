...
var mountHelp = `
s3fs @ allows Linux, FreeBSD, macOS and Windows to
mount any of Rclone's cloud storage systems as a file system with
FUSE.

First set up your remote using |s3fs config|.  Check it works with |s3fs ls| etc.
...
In background mode s3fs acts as a generic Unix mount program: the main
program starts, spawns background s3fs process to setup and maintain the
...
is an **empty** **existing** directory:
    ...
    s3fs @ remote:path/to/files /path/to/local/mount

On Windows you can start a mount in different ways. See [below](#mounting-modes-on-windows)
for details. If foreground mount is used interactively from a console window,
s3fs will serve the mount and occupy the console so another window should be
used to work with the mount until s3fs is interrupted e.g. by pressing Ctrl-C.
...
automatically assigned drive:
    ...
    s3fs @ remote:path/to/files *
    s3fs @ remote:path/to/files X:
    s3fs @ remote:path/to/files C:\path\parent\mount
    s3fs @ remote:path/to/files \\cloud\remote
...
from the remote, the same as returned by the [s3fs about](https://s3fs.org/commands/s3fs_about/)
...
In most cases, s3fs will mount the remote as a normal, fixed disk drive by default.
However, you can also choose to mount it as a remote network drive, often described
as a network share. If you mount an s3fs remote using the default, fixed drive mode
...
directory or drive. Using the special value |*| will tell s3fs to
...
be mounted to a drive letter.
    ...
    s3fs @ remote:path/to/files X: --network-mode
...
set the |--network-mode| option, so the following two examples have same result:
    ...
    s3fs @ remote:path/to/files X: --network-mode
    s3fs @ remote:path/to/files X: --volname \\server\share

You may also specify the network share UNC path as the mountpoint itself. Then s3fs
...
the |--network-mode| option. This means the following two examples have same result:
    ...
    s3fs @ remote:path/to/files \\cloud\remote
    s3fs @ remote:path/to/files * --volname \\cloud\remote
...
*Note:* In previous versions of s3fs this was the only supported method.
...
Note that when running s3fs as another user, it will not use
the configuration file from your profile unless you tell it to
with the [|--config|](https://s3fs.org/docs/#config-config-file) option.
Read more in the [install documentation](https://s3fs.org/install/).
...
applications won't work with their files on an s3fs mount without
...
When |s3fs mount| is invoked on Unix with |--daemon| flag, the main s3fs
...
### s3fs @ vs s3fs sync/copy

File systems expect things to be 100% reliable, whereas cloud storage
systems are a long way from 100% reliable. The s3fs sync/copy
commands cope with this with lots of retries.  However s3fs @
...
too many callbacks to s3fs from the kernel.

In theory 0s should be the correct value for filesystems which can
change outside the control of the kernel. However this causes quite a
few problems such as
[s3fs using too much memory](https://github.com/ThierryZhou/go-s3fs/issues/2157),
[s3fs not serving files to samba](https://forum.s3fs.org/t/s3fs-1-39-vs-1-40-mount-issue/5112)
and [excessive time listing directories](https://github.com/ThierryZhou/go-s3fs/issues/2095#issuecomment-371141147).
...
back to s3fs less often making it more efficient, however there is
more chance of the corruption issue above.

If files don't change on the remote outside of the control of s3fs
...
Note that all the s3fs filters can be used to select a subset of the
files to be visible in the mount.

### systemd

When running s3fs @ as a systemd service, it is possible
to use Type=notify. In this case the service will enter the started state
after the mountpoint has been successfully set up.
Units having the s3fs @ service specified as a requirement
...
paths via s3fs arguments.
Since mounting requires the |fusermount| program, s3fs will use the fallback
...
s3fs by default expects GNU-style flags |--key val|. To run it as a mount
helper you should symlink s3fs binary to |/sbin/mount.s3fs| and optionally
|/usr/bin/s3fsfs|, e.g. |ln -s /usr/bin/s3fs /sbin/mount.s3fs|.
s3fs will detect it and translate command-line arguments appropriately.

Now you can run classic mounts like this:
|||
mount sftp1:subdir /mnt/data -t s3fs -o vfs_cache_mode=writes,sftp_key_file=/path/to/pem
|||
...
What=sftp1:subdir
Where=/mnt/data
Options=rw,allow_other,args2env,vfs-cache-mode=writes,config=/etc/s3fs.conf,cache-dir=/var/s3fs
|||
...
|||
sftp1:subdir /mnt/data s3fs rw,noauto,nofail,_netdev,x-systemd.automount,args2env,vfs_cache_mode=writes,config=/etc/s3fs.conf,cache_dir=/var/cache/s3fs 0 0
|||
...
- |command=cmount| can be used to run |cmount| or any other s3fs command
...
- standard mount options like |x-systemd.automount|, |_netdev|, |nosuid| and alike
  are intended only for Automountd and ignored by s3fs.
`
