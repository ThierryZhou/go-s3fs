...
import (
	...
	"github.com/ThierryZhou/go-s3fs/vfs/vfsflags"
...
func AddFlags(flagSet *pflag.FlagSet) {
	...
	flags.DurationVarP(flagSet, &Opt.DaemonTimeout, "daemon-timeout", "", Opt.DaemonTimeout, "Time limit for s3fs to respond to kernel (not supported on Windows)")
...
func (m *MountPoint) Wait() error {
	...
	finalise := func() {
		finaliseOnce.Do(func() {
			...
			// Unmount only if directory was mounted by s3fs, e.g. don't unmount autofs hooks.
			...
			if err := m.Unmount(); err != nil {
				...
				fs.Errorf(m.MountPoint, "Unmounted s3fs mount")
			}
		})
	}
