...
// CheckMountReady checks whether mountpoint is mounted by s3fs.
// Only mounts with type "s3fs" or "fuse.s3fs" count.
func CheckMountReady(mountpoint string) error {
	...
	for _, entry := range entries {
		if entry.Dir == mountpointAbs && strings.Contains(entry.Type, "s3fs") {
			return nil
		}
	}
...
// WaitMountReady waits until mountpoint is mounted by s3fs.
